"""SSE streaming manager for real-time audit workflow updates.

Provides an event bus that workflow graph nodes write to and that API
...
from compliance_audit.models import AuditEvent
...
EVENT_CLASSIFYING = "classifying"
EVENT_CLASSIFICATION_DONE = "classification_done"
EVENT_CHECKING_SOX = "checking_sox"
EVENT_CHECKING_GDPR = "checking_gdpr"
EVENT_CHECKING_SOC2 = "checking_soc2"
EVENT_CHECK_COMPLETE = "check_complete"
EVENT_SCORING = "scoring"
EVENT_SCORING_DONE = "scoring_done"
EVENT_REMEDIATING = "remediating"
EVENT_REMEDIATION_DONE = "remediation_done"
...
class AuditEventStream:
    """In-memory pub/sub for audit session SSE events.

    Each audit session gets its own ``asyncio.Queue`` so that multiple
    SSE subscribers can consume events independently.
    """

    def __init__(self, max_queue_size: int = 256) -> None:
        self._queues: dict[str, list[asyncio.Queue[AuditEvent | None]]] = {}
    ...
    async def emit(
        ...
        Returns the constructed :class:`AuditEvent` for convenience.
        """
        event = AuditEvent(
    ...
    async def subscribe(self, session_id: str) -> AsyncIterator[AuditEvent]:
        ...
        """
        queue: asyncio.Queue[AuditEvent | None] = asyncio.Queue(
    ...
    def get_history(self, session_id: str) -> list[AuditEvent]:
